...
func _unhandled_input(event):
	if (event.is_action_pressed("ui_accept")):
		...
		$Sprite2D.get_texture().get_image().save_png("res://screenshots/debug" + get_parent().name + ".png")


func destroy(pos : Vector2, radius : float):
	# Collision rebuild thread!
	var thread := Thread.new()
	var error = thread.start(Callable(self, "_threaded_rebuild_collisions").bind([pos, radius]))
	if error != OK:
...
func _threaded_rebuild_collisions(arguments: Array):
	call_deferred("rebuild_collisions_from_geometry", arguments)
...
func rebuild_collisions_from_geometry(arguments : Array):
	...
	for collision_polygon in collision_holder.get_children():
		...
		for i in range(clipped_polygons.size()):
			...
			# ie, PoolVector2Array(Geometry2D.clip_polygons_2d(points_arc, collision_polygon.polygon))
